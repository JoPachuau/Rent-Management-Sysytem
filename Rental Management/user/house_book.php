...
<body>
    ...
    <form action="model/add_house_form.php" method="GET">
        ...
        <br>
        <div class="">
            <label for="selected_house'">Select House Preference</label>
            <?php
            $sql = 'SELECT house_name From add_home';
            $res = mysqli_query($con, $sql);
            echo "<select id='room-selection' name='selected_house' required>";
            while ($row = mysqli_fetch_array($res)) {
                echo "<option id='room-selection' >Choose Houses from the List</option>";
                echo "<option type='text'  name='selected_house' >$row[0]</option>";
            }
            echo "</select>";
            ?>
        </div>
